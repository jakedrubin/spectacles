...
Requirements:
    pip install google-genai pandas Pillow python-dotenv

Usage:
    Set your API key in conda env:
        conda env config vars set GOOGLE_API_KEY='your-key-here'

    Run:
        python gemini_recommend.py --face-image path/to/face.jpg --top-k 5

    Optionally point to a custom frame catalog:
        python gemini_recommend.py --face-image path/to/face.jpg --catalog data/frame_catalog.csv --top-k 3
"""
...
# Configuration
# ---------------------------------------------------------------------------
MODEL = "gemini-2.5-flash"          # adjust to whichever model you want
DEFAULT_CATALOG = Path(__file__).parent.parent / "data" / "frame_catalog.csv"
DEFAULT_FACE_IMAGE = Path(__file__).parent.parent / "data" / "faces/noah_face.jpg"
...
def cli():
    ...
    parser.add_argument(
        "--face-image", type=str, default=str(DEFAULT_FACE_IMAGE),
        help="Path to a photo of the person's face (jpg/png/webp) (default: data/faces/noah_face.jpg).",
    )
    parser.add_argument(
